...
- To route Codex through an OpenAI-compatible proxy such as ChatMock, set env.OPENAI_BASE_URL to the proxy's /v1 URL and env.OPENAI_API_KEY to any non-empty placeholder value accepted by that proxy. Paperclip will automatically inject a custom Codex provider config so proxy runs avoid the built-in OpenAI provider's websocket path.
