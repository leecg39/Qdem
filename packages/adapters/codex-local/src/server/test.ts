...
function readNonEmptyEnvValue(env: Record<string, string> | NodeJS.ProcessEnv, key: string): string | null {
  const raw = env[key];
  return typeof raw === "string" && raw.trim().length > 0 ? raw.trim() : null;
}

const PAPERCLIP_PROXY_PROVIDER_ID = "paperclip-openai-proxy";

function hasExplicitProviderOverride(args: string[]): boolean {
  return args.some((arg) =>
    arg === "--base-url" ||
    arg.startsWith("--base-url=") ||
    arg.includes("model_provider=") ||
    arg.includes("model_providers."),
  );
}

function buildProxyProviderArgs(env: Record<string, string> | NodeJS.ProcessEnv, extraArgs: string[]): string[] {
  const baseUrl = readNonEmptyEnvValue(env, "OPENAI_BASE_URL");
  if (!baseUrl || hasExplicitProviderOverride(extraArgs)) return [];

  return [
    "-c",
    `model_provider=${JSON.stringify(PAPERCLIP_PROXY_PROVIDER_ID)}`,
    "-c",
    `model_providers.${PAPERCLIP_PROXY_PROVIDER_ID}={ name = ${JSON.stringify("Paperclip OpenAI Proxy")}, base_url = ${JSON.stringify(baseUrl)}, env_key = ${JSON.stringify("OPENAI_API_KEY")}, wire_api = "responses", supports_websockets = false }`,
  ];
}
...
export async function testEnvironment(
  ...
  if (canRunProbe) {
    if (!commandLooksLike(command, "codex")) {
      ...
      const proxyProviderArgs = buildProxyProviderArgs(runtimeEnv, extraArgs);
      ...
      if (proxyProviderArgs.length > 0) args.push(...proxyProviderArgs);
