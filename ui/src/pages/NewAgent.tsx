...
export function NewAgent() {
  ...
  const createAgent = useMutation({
    ...
    onError: (error) => {
      setFormError(error instanceof Error ? error.message : "에이전트 생성에 실패했습니다");
    },
  });
  ...
  function handleSubmit() {
    ...
    if (configValues.adapterType === "opencode_local") {
      ...
      if (adapterModelsLoading || adapterModelsFetching) {
        setFormError("OpenCode 모델을 불러오는 중입니다. 잠시 후 다시 시도하세요.");
        return;
      }
      ...
      if (!discovered.some((entry) => entry.id === selectedModel)) {
        setFormError(
          discovered.length === 0
            ? "OpenCode 모델이 감지되지 않았습니다. `opencode models`를 실행하고 공급자를 인증하세요."
            : `설정된 OpenCode 모델을 사용할 수 없습니다: ${selectedModel}`,
        );
        return;
      }
    }
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="border border-border">
              ...
              >
                {currentReportsTo ? (
                  <>
                    ...
                    {`보고 대상: ${currentReportsTo.name}`}
                  </>
                ) : (
                  <>
                    <User className="h-3 w-3 text-muted-foreground" />
                    {isFirstAgent ? "보고 대상: 없음 (CEO)" : "보고 대상..."}
                  </>
                )}
              </button>
            </PopoverTrigger>
        ...
        <div className="border-t border-border px-4 py-3">
          {isFirstAgent && (
            <p className="text-xs text-muted-foreground mb-2">이 에이전트가 CEO가 됩니다</p>
          )}
          {formError && (
