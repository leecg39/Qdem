              ...
              <div className="flex items-center justify-between">
                ...
                {data.summary.budgetCents > 0 && (
                  <p className="text-sm text-muted-foreground">
                    {data.summary.utilizationPercent}% 사용됨
                  </p>
                )}
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <Card>
              <CardContent className="p-4">
                <h3 className="text-sm font-semibold mb-3">에이전트별</h3>
                      ...
                      >
                        ...
                        <div className="text-right shrink-0 ml-2 tabular-nums">
                          ...
                          <span className="text-xs text-muted-foreground block">
                            입력 {formatTokens(row.inputTokens)} / 출력 {formatTokens(row.outputTokens)} 토큰
                          </span>
                          {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) && (
                            <span className="text-xs text-muted-foreground block">
                              {row.apiRunCount > 0 ? `API 실행: ${row.apiRunCount}` : null}
                              {row.apiRunCount > 0 && row.subscriptionRunCount > 0 ? " | " : null}
                              {row.subscriptionRunCount > 0
                                ? `구독 실행: ${row.subscriptionRunCount} (입력 ${formatTokens(row.subscriptionInputTokens)} / 출력 ${formatTokens(row.subscriptionOutputTokens)} 토큰)`
                                : null}
                            </span>
                          )}
                        </div>
            ...
            <Card>
              <CardContent className="p-4">
                <h3 className="text-sm font-semibold mb-3">프로젝트별</h3>
                {data.byProject.length === 0 ? (
                  <p className="text-sm text-muted-foreground">프로젝트에 귀속된 비용 데이터가 없습니다.</p>
                ) : (
                  <div className="space-y-2">
                    {data.byProject.map((row) => (
                      ...
                      >
                        <span className="truncate">
                          {row.projectName ?? row.projectId ?? "미귀속"}
                        </span>
