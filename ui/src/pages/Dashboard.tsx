...
export function Dashboard() {
  ...
  if (!selectedCompanyId) {
    if (companies.length === 0) {
      return (
        <EmptyState
          ...
          message="Paperclip에 환영합니다. 첫 번째 회사와 에이전트를 설정하여 시작하세요."
          action="시작하기"
    ...
    return (
      <EmptyState icon={LayoutDashboard} message="대시보드를 보려면 회사를 만들거나 선택하세요." />
    );
  }
              ...
              description={
                <span>
                  {data.costs.monthBudgetCents > 0
                    ? `예산 ${formatCents(data.costs.monthBudgetCents)} 중 ${data.costs.monthUtilizationPercent}%`
                    : "예산 무제한"}
                </span>
              }
            />
