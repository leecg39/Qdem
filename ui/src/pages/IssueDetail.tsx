...
import { relativeTime, cn, formatTokens, formatCostWon } from "../lib/utils";
...
export function IssueDetail() {
  ...
  const sourceBreadcrumb = useMemo(
    () => readIssueDetailBreadcrumb(location.state) ?? { label: "이슈", href: "/issues" },
    [location.state],
  );
  ...
  const attachmentUploadButton = (
    <>
      ...
      >
        ...
        {uploadAttachment.isPending || importMarkdownDocument.isPending ? "업로드 중..." : "첨부파일 업로드"}
      </Button>
    </>
  );
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      {linkedRuns && linkedRuns.length > 0 && (
        ...
        >
          <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
            <span className="text-sm font-medium text-muted-foreground">비용 요약</span>
            <ChevronDown
                ...
                <div className="flex flex-wrap gap-3 text-xs text-muted-foreground tabular-nums">
                  {issueCostSummary.hasCost && (
                    <span className="font-medium text-foreground">
                      {formatCostWon(issueCostSummary.cost)}
                    </span>
                  )}
