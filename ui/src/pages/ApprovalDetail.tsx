...
export function ApprovalDetail() {
  ...
  if (!approval) return <p className="text-sm text-muted-foreground">승인을 찾을 수 없습니다.</p>;
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">승인이 확정되었습니다</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  요청 에이전트에게 이 승인 및 연결된 이슈를 검토하도록 알림이 전송되었습니다.
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">결정 사유: {approval.decisionNote}</p>
          )}
        </div>
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              연결된 이슈는 요청 에이전트가 후속 조치를 완료하고 닫을 때까지 열린 상태로 유지됩니다.
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm("거부된 에이전트를 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.")) return;
