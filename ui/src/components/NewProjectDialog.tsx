...
export function NewProjectDialog() {
  ...
  async function handleSubmit() {
    ...
    if (repoRequired && !isGitHubRepoUrl(repoUrl)) {
      setWorkspaceError("저장소 워크스페이스는 유효한 GitHub 저장소 URL을 사용해야 합니다.");
      return;
    }
    ...
    >
      ...
      >
        ...
        <div className="px-4 pb-3 space-y-3 border-t border-border">
          <div className="pt-3">
            <p className="text-sm font-medium">이 프로젝트에서 작업이 진행될 위치는 어디인가요?</p>
            <p className="text-xs text-muted-foreground">로컬 폴더 및/또는 GitHub 저장소 워크스페이스 힌트를 추가하세요.</p>
          </div>
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">이 머신의 전체 경로를 사용하세요.</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">GitHub URL을 붙여넣으세요.</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">로컬 + 저장소 힌트를 설정하세요.</p>
            </button>
          </div>

          {(workspaceSetup === "local" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">로컬 폴더 (전체 경로)</label>
          ...
          {(workspaceSetup === "repo" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">GitHub 저장소 URL</label>
              <input
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">프로젝트 생성에 실패했습니다.</p>
          ) : (
            <span />
          )}
          <Button
