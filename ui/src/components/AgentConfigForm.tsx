    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">신원</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">신원</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            <Field label="직함" hint={help.title}>
              <DraftInput
                ...
                immediate
                className={inputClass}
                placeholder="예: VP of Engineering"
              />
            </Field>
            <Field label="역할" hint={help.capabilities}>
              <MarkdownEditor
            ...
            {isLocal && (
              <>
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  프롬프트 템플릿은 매 하트비트마다 재실행됩니다. 반복적인 토큰 비용과 캐시 낭비를 줄이기 위해 간결하고 동적으로 유지하세요.
                </div>
              </>
            )}
          </div>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          ...
          {/* Working directory */}
          {isLocal && (
            <Field label="작업 디렉토리" hint={help.cwd}>
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                프롬프트 템플릿은 매 하트비트마다 재실행됩니다. <code>{"{{ context.* }}"}</code> 또는 <code>{"{{ run.* }}"}</code>와 같은 소규모 작업 프레이밍 및 변수를 선호하고, 고정된 지침을 반복하지 마세요.
              </div>
            </>
          )}
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">권한 &amp; 설정</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">권한 &amp; 설정</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label="명령어" hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        검색이 활성화된 경우 Codex가 `minimal` 사고를 거부할 수 있습니다.
                      </p>
                    )}
                </>
              )}
              <Field label="부트스트랩 프롬프트 (첫 실행)" hint={help.bootstrapPrompt}>
                <MarkdownEditor
                  value={
                    isCreate
                      ? val!.bootstrapPrompt
                  ...
                  placeholder="첫 실행을 위한 선택적 초기 설정 프롬프트"
                  contentClassName="min-h-[44px] text-sm font-mono"
                  imageUploadHandler={async (file) => {
              ...
              <div className="rounded-md border border-sky-500/25 bg-sky-500/10 px-3 py-2 text-xs text-sky-100">
                부트스트랩 프롬프트는 새 세션에서만 전송됩니다. 안정적인 설정, 습관, 재사용 가능한 긴 지침을 여기에 넣으세요. 자주 변경하면 새 세션이 이를 재실행해야 하므로 세션 재사용의 가치가 줄어듭니다.
              </div>
              {adapterType === "claude_local" && (
                <ClaudeLocalAdvancedFields {...adapterFieldProps} />
              )}

              <Field label="추가 인수 (쉼표로 구분)" hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
              ...
              {!isCreate && (
                <>
                  <Field label="타임아웃 (초)" hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label="인터럽트 유예 기간 (초)" hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
        {ADAPTER_DISPLAY_LIST.map((item) => (
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-[10px] text-muted-foreground">출시 예정</span>
            )}
          </button>
        ))}
      </PopoverContent>
...
function EnvVarEditor({
  ...
  async function sealRow(i: number) {
    ...
    try {
      ...
      setSealError(err instanceof Error ? err.message : "시크릿 생성에 실패했습니다");
    }
  }
  ...
  return (
    <div className="space-y-1.5">
      ...
      <p className="text-[11px] text-muted-foreground/60">
        PAPERCLIP_* 변수는 런타임에 자동으로 주입됩니다.
      </p>
    </div>
  );
}
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          ...
          <div className="max-h-[240px] overflow-y-auto">
            ...
            {filteredModels.length === 0 && (
              <p className="px-2 py-1.5 text-xs text-muted-foreground">모델을 찾을 수 없습니다.</p>
            )}
          </div>
        </PopoverContent>
      </Popover>
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label="사고 수준" hint={help.thinkingEffort}>
