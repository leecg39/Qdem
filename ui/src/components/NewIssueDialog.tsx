...
export function NewIssueDialog() {
  ...
  const uploadDescriptionImage = useMutation({
    mutationFn: async (file: File) => {
      if (!effectiveCompanyId) throw new Error("회사가 선택되지 않았습니다");
          ...
          <div className="px-4 pb-2 shrink-0">
            <div className="flex items-center justify-between rounded-md border border-border px-3 py-2">
              <div className="space-y-0.5">
                <div className="text-xs font-medium">격리된 이슈 체크아웃 사용</div>
                <div className="text-[11px] text-muted-foreground">
                  프로젝트의 기본 체크아웃 대신 이슈 전용 실행 워크스페이스를 생성합니다.
                </div>
              </div>
              <button
                className={cn(
          ...
          <div className="px-4 pb-2 shrink-0">
            ...
            {assigneeOptionsOpen && (
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                ...
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">사고 강도</div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                ...
                <span>{createIssue.isPending ? "생성 중..." : "이슈 만들기"}</span>
              </span>
            </Button>
          </div>
        </div>
