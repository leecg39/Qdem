...
## Using ChatMock with Codex locally

If you want local Codex-backed agents to run through ChatMock instead of talking directly to `api.openai.com`, start ChatMock and point Codex at its OpenAI-compatible `/v1` endpoint.

1. Install and verify ChatMock:

```sh
uv tool install "git+https://github.com/RayBytes/ChatMock.git"
chatmock info
python3 scripts/chatmock/patch-installed-chatmock.py
```
...
If your ChatMock build only exposes `/v1/chat/completions`, Codex will not work through it directly. Paperclip's `codex_local` adapter expects a Codex-compatible `/v1/responses` route, so use a ChatMock build (or local compatibility patch) that exposes `/v1/responses` and returns a Codex-friendly `/v1/models` payload.

3. Add the Codex-compatible proxy settings in `server/.env` before `pnpm dev`:

```sh
cat > server/.env <<'EOF'
OPENAI_BASE_URL=http://127.0.0.1:8000/v1
OPENAI_API_KEY=chatmock-local
EOF

pnpm dev
```

Paperclip's `codex_local` adapter forwards those environment variables to the `codex` CLI. The placeholder API key is only used to satisfy OpenAI-compatible clients such as ChatMock.
